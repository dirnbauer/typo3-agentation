...
`;const c=r.some(M=>M.type==="navigation"),d=r.some(M=>M.type==="hero"),p=r.some(M=>M.type==="sidebar"),w=r.some(M=>M.type==="footer"),g=r.filter(M=>M.type==="card"),$=r.filter(M=>M.type==="form"),k=r.filter(M=>M.type==="table"),j=r.filter(M=>M.type==="modal");if(c&&(l+=`- Top navigation bar with logo + nav links + CTA
...
`}c&&(l+="- Navigation: `position: sticky; top: 0; z-index: 50;`\n")}return l}function B_(e,t="standard",n){const{sections:o}=e,r=[];for(const p of o){const w=p.originalRect,g=p.currentRect,$=Math.abs(w.x-g.x)>1||Math.abs(w.y-g.y)>1,k=Math.abs(w.width-g.width)>1||Math.abs(w.height-g.height)>1;if(!$&&!k){t==="forensic"&&r.push({section:p,posMoved:!1,sizeChanged:!1});continue}r.push({section:p,posMoved:$,sizeChanged:k})}if(r.length===0||t!=="forensic"&&r.every(p=>!p.posMoved&&!p.sizeChanged))return"";let l=`## Suggested Layout Changes
...
`);const d=p=>o.map(w=>({label:w.label,selector:w.selector,rect:p==="original"?w.originalRect:w.currentRect}));l+=`**Changes:**
`;for(const{section:p,posMoved:w,sizeChanged:g}of r){const $=p.originalRect,k=p.currentRect;if(!w&&!g){l+=`- ${p.label} — unchanged at (${Math.round(k.x)}, ${Math.round(k.y)}) ${Math.round(k.width)}×${Math.round(k.height)}px
`;continue}if(t==="compact"){w&&g?l+=`- Suggested: move **${p.label}** to (${Math.round(k.x)}, ${Math.round(k.y)}) ${Math.round(k.width)}×${Math.round(k.height)}px
`:w?l+=`- Suggested: move **${p.label}** to (${Math.round(k.x)}, ${Math.round(k.y)})
`:l+=`- Suggested: resize **${p.label}** to ${Math.round(k.width)}×${Math.round(k.height)}px
`;continue}if(w&&g?l+=`- Suggested: move and resize **${p.label}**
`:w?l+=`- Suggested: move **${p.label}**
`:l+=`- Suggested: resize **${p.label}** from ${Math.round($.width)}×${Math.round($.height)}px to ${Math.round(k.width)}×${Math.round(k.height)}px
`,w){const M=Fc($,d("original")),h=Fc(k,d("current")),y=g?{width:$.width,height:$.height}:void 0;l+=`  - Currently ${z1(M,{x:$.x,y:$.y},y)}
...
`)}if(t!=="compact"){const p=r.filter(g=>g.posMoved).map(g=>({label:g.section.label,originalRect:g.section.originalRect,currentRect:g.section.currentRect})),w=O1(p);if(w.length>0){l+=`
### Layout Summary
`;for(const g of w)l+=`- ${g}
`}}if(t!=="compact"&&o.length>1){l+=`
### All Sections (current positions)
`;const p=[...o].sort((w,g)=>Math.abs(w.currentRect.y-g.currentRect.y)<20?w.currentRect.x-g.currentRect.x:w.currentRect.y-g.currentRect.y);for(const w of p){const g=w.currentRect,$=Math.abs(g.x-w.originalRect.x)>1||Math.abs(g.y-w.originalRect.y)>1||Math.abs(g.width-w.originalRect.width)>1||Math.abs(g.height-w.originalRect.height)>1;l+=`- ${w.label}: \`${Math.round(g.width)}×${Math.round(g.height)}px\` at \`(${Math.round(g.x)}, ${Math.round(g.y)})\`${$?" ← suggested":""}
...
`)}
    ...
    }`].join(""),document.head.appendChild(e)};Ox();function Uo(e,t){let n=document.elementFromPoint(e,t);if(!n)return null;for(;n?.shadowRoot;){const o=n.shadowRoot.elementFromPoint(e,t);if(!o||o===n)break;n=o}return n}function Xa(e){let t=e;for(;t&&t!==document.body;){const o=window.getComputedStyle(t).position;if(o==="fixed"||o==="sticky")return!0;t=t.parentElement}return!1}function Yo(e){return e.status!=="resolved"&&e.status!=="dismissed"}function Yl(e){const t=Uc(e),n=t.found?t:kx(e);if(n.found&&n.source)return bx(n.source,"path")}function Ax({demoAnnotations:e,demoDelay:t=1e3,enableDemoMode:n=!1,onAnnotationAdd:o,onAnnotationDelete:r,onAnnotationUpdate:l,onAnnotationsClear:i,onCopy:a,onSubmit:c,copyToClipboard:d=!0,endpoint:p,sessionId:w,onSessionCreated:g,webhookUrl:$,className:k}={}){const[j,M]=f.useState(!1),[h,y]=f.useState([]),[b,N]=f.useState(!0),[W,Y]=f.useState(()=>J1()),[D,Q]=f.useState(!1),he=f.useRef(null);f.useEffect(()=>{const u=x=>{const v=he.current;v&&v.contains(x.target)&&x.stopPropagation()},m=["mousedown","click","pointerdown"];return m.forEach(x=>document.body.addEventListener(x,u)),()=>{m.forEach(x=>document.body.removeEventListener(x,u))}},[]);const[q,de]=f.useState(!1),[Be,Ae]=f.useState(!1),[ke,Ne]=f.useState(null),[oe,lt]=f.useState({x:0,y:0}),[I,U]=f.useState(null),[G,Se]=f.useState(!1),[Fe,wt]=f.useState("idle"),[Pt,Dt]=f.useState(!1),[Bt,zt]=f.useState(!1),[Vn,ao]=f.useState(null),[co,on]=f.useState(null),[Gt,_n]=f.useState([]),[Sn,uo]=f.useState(null),[Kn,_o]=f.useState(null),[T,ie]=f.useState(null),[Me,ge]=f.useState(null),[Ce,Ye]=f.useState([]),[He,Ie]=f.useState(0),[Xe,at]=f.useState(!1),[ue,C]=f.useState(!1),[E,B]=f.useState(!1),[O,se]=f.useState(!1),[K,F]=f.useState(!1),[we,be]=f.useState("main"),[ze,Ke]=f.useState(!1),[ae,We]=f.useState(!1),[Oe,Le]=f.useState(!1),[re,Ge]=f.useState([]),[je,pe]=f.useState(null),Qe=f.useRef(!1),[$e,fn]=f.useState(!1),[qt,rn]=f.useState(!1),[Bn,hn]=f.useState(1),[ar,sl]=f.useState("new-page"),[mt,Gn]=f.useState(""),[ll,Cp]=f.useState(!1),[xe,pn]=f.useState(null),Qi=f.useRef(!1),Vi=f.useRef({rearrange:null,placements:[]}),Do=f.useRef({rearrange:null,placements:[]}),[jp,Uu]=f.useState(0),[Mp,Ep]=f.useState(0),[Ip,Ki]=f.useState(0),[$p,Yu]=f.useState(0),qr=f.useRef(new Set),il=f.useRef(new Set),Cn=f.useRef(null),al=f.useRef(),Xu=ae&&j&&!Oe&&$e;f.useEffect(()=>{if(Xu){rn(!1);const u=Lr(()=>{rn(!0)});return()=>cancelAnimationFrame(u)}else rn(!1)},[Xu]);const Jr=f.useRef(new Map),Zr=f.useRef(new Map),es=f.useRef(),[jn,Gi]=f.useState(!1),[mn,Np]=f.useState([]),Lp=f.useRef(mn);Lp.current=mn;const[Qu,Hx]=f.useState(null),qi=f.useRef(null);f.useRef(!1),f.useRef([]),f.useRef(0),f.useRef(null),f.useRef(null),f.useRef(1);const[Vu,Ku]=f.useState(!1),cr=f.useRef(null),[bt,ur]=f.useState([]),zn=f.useRef({cmd:!1,shift:!1}),Ot=()=>{Ke(!0)},Rp=()=>{Ke(!1)},Tp=()=>{Vu||(cr.current=fe(()=>Ku(!0),850))},Pp=()=>{cr.current&&(clearTimeout(cr.current),cr.current=null),Ku(!1),Rp()};f.useEffect(()=>()=>{cr.current&&clearTimeout(cr.current)},[]);const[De,Dp]=f.useState(()=>{try{const u=JSON.parse(localStorage.getItem("feedback-toolbar-settings")??"");return{...Ya,...u,annotationColorId:Ps.find(m=>m.id===u.annotationColorId)?u.annotationColorId:Ya.annotationColorId}}catch{return Ya}}),[On,Gu]=f.useState(!0),[qu,Ju]=f.useState(!1),Bp=()=>{he.current?.classList.add(A.disableTransitions),Gu(u=>!u),Lr(()=>{he.current?.classList.remove(A.disableTransitions)})},zp=!1,Bo="off",[Ct,Ji]=f.useState(w??null),Zu=f.useRef(!1),[Mn,zo]=f.useState(p?"connecting":"disconnected"),[ct,Zi]=f.useState(null),[Oo,ed]=f.useState(!1),[dr,td]=f.useState(null),ea=f.useRef(!1),[nd,ts]=f.useState(new Set),[od,cl]=f.useState(new Set),[ns,ul]=f.useState(!1),[Op,_r]=f.useState(!1),[qn,rd]=f.useState(!1),fr=f.useRef(null),An=f.useRef(null),os=f.useRef(null),rs=f.useRef(null),dl=f.useRef(!1),sd=f.useRef(0),_l=f.useRef(null),ld=f.useRef(null),ta=8,Ap=50,id=f.useRef(null),ad=f.useRef(null),ss=f.useRef(null),me=typeof window<"u"?window.location.pathname:"/";f.useEffect(()=>{if(O)F(!0);else{Ke(!1),be("main");const u=fe(()=>F(!1),0);return()=>clearTimeout(u)}},[O]);const na=j&&b&&!ae;f.useEffect(()=>{if(na){Ae(!1),de(!0),ts(new Set);const u=fe(()=>{ts(m=>{const x=new Set(m);return h.forEach(v=>x.add(v.id)),x})},350);return()=>clearTimeout(u)}else if(q){Ae(!0);const u=fe(()=>{de(!1),Ae(!1)},250);return()=>clearTimeout(u)}},[na]),f.useEffect(()=>{C(!0),Ie(window.scrollY);const u=za(me);y(u.filter(Yo)),V_||(Ju(!0),V_=!0,fe(()=>Ju(!1),750));try{const m=localStorage.getItem("feedback-toolbar-theme");m!==null&&Gu(m==="dark")}catch{}try{const m=localStorage.getItem("feedback-toolbar-position");if(m){const x=JSON.parse(m);typeof x.x=="number"&&typeof x.y=="number"&&Zi(x)}}catch{}},[me]),f.useEffect(()=>{ue&&localStorage.setItem("feedback-toolbar-settings",JSON.stringify(De))},[De,ue]),f.useEffect(()=>{ue&&localStorage.setItem("feedback-toolbar-theme",On?"dark":"light")},[On,ue]);const cd=f.useRef(!1);f.useEffect(()=>{const u=cd.current;cd.current=Oo,u&&!Oo&&ct&&ue&&localStorage.setItem("feedback-toolbar-position",JSON.stringify(ct))},[Oo,ct,ue]),f.useEffect(()=>{if(!p||!ue||Zu.current)return;Zu.current=!0,zo("connecting"),(async()=>{try{const m=G1(me),x=w||m;let v=!1;if(x)try{const S=await O_(p,x);Ji(S.id),zo("connected"),Oa(me,S.id),v=!0;const P=za(me),X=new Set(S.annotations.map(ne=>ne.id)),V=P.filter(ne=>!X.has(ne.id));if(V.length>0){const le=`${typeof window<"u"?window.location.origin:""}${me}`,ve=(await Promise.allSettled(V.map(_e=>yr(p,S.id,{..._e,sessionId:S.id,url:le})))).map((_e,Z)=>_e.status==="fulfilled"?_e.value:(console.warn("[Agentation] Failed to sync annotation:",_e.reason),V[Z])),Te=[...S.annotations,...ve];y(Te.filter(Yo)),ys(me,Te.filter(Yo),S.id)}else y(S.annotations.filter(Yo)),ys(me,S.annotations.filter(Yo),S.id)}catch(S){console.warn("[Agentation] Could not join session, creating new:",S),q1(me)}if(!v){const S=typeof window<"u"?window.location.href:"/",P=await Aa(p,S);Ji(P.id),zo("connected"),Oa(me,P.id),g?.(P.id);const X=F1(),V=typeof window<"u"?window.location.origin:"",ne=[];for(const[le,ce]of X){const ve=ce.filter(Z=>!Z._syncedTo);if(ve.length===0)continue;const Te=`${V}${le}`,_e=le===me;ne.push((async()=>{try{const Z=_e?P:await Aa(p,Te),jt=(await Promise.allSettled(ve.map(qe=>yr(p,Z.id,{...qe,sessionId:Z.id,url:Te})))).map((qe,yt)=>qe.status==="fulfilled"?qe.value:(console.warn("[Agentation] Failed to sync annotation:",qe.reason),ve[yt])).filter(Yo);if(ys(le,jt,Z.id),_e){const qe=new Set(ve.map(yt=>yt.id));y(yt=>{const ye=yt.filter(Ee=>!qe.has(Ee.id));return[...jt,...ye]})}}catch(Z){console.warn(`[Agentation] Failed to sync annotations for ${le}:`,Z)}})())}await Promise.allSettled(ne)}}catch(m){zo("disconnected"),console.warn("[Agentation] Failed to initialize session, using local storage:",m)}})()},[p,w,ue,g,me]),f.useEffect(()=>{if(!p||!ue)return;const u=async()=>{try{(await fetch(`${p}/health`)).ok?zo("connected"):zo("disconnected")}catch{zo("disconnected")}};u();const m=Yg(u,1e4);return()=>clearInterval(m)},[p,ue]),f.useEffect(()=>{if(!p||!ue||!Ct)return;const u=new EventSource(`${p}/sessions/${Ct}/events`),m=["resolved","dismissed"],x=v=>{try{const S=JSON.parse(v.data);if(m.includes(S.payload?.status)){const P=S.payload.id,X=S.payload.kind;if(X==="placement"){for(const[V,ne]of Jr.current)if(ne===P){Jr.current.delete(V),Ge(le=>le.filter(ce=>ce.id!==V));break}}else if(X==="rearrange"){for(const[V,ne]of Zr.current)if(ne===P){Zr.current.delete(V),pn(le=>{if(!le)return null;const ce=le.sections.filter(ve=>ve.id!==V);return ce.length===0?null:{...le,sections:ce}});break}}else cl(V=>new Set(V).add(P)),fe(()=>{y(V=>V.filter(ne=>ne.id!==P)),cl(V=>{const ne=new Set(V);return ne.delete(P),ne})},150)}}catch{}};return u.addEventListener("annotation.updated",x),()=>{u.removeEventListener("annotation.updated",x),u.close()}},[p,ue,Ct]),f.useEffect(()=>{if(!p||!ue)return;const u=ld.current==="disconnected",m=Mn==="connected";ld.current=Mn,u&&m&&(async()=>{try{const v=za(me);if(v.length===0)return;const P=`${typeof window<"u"?window.location.origin:""}${me}`;let X=Ct,V=[];if(X)try{V=(await O_(p,X)).annotations}catch{X=null}X||(X=(await Aa(p,P)).id,Ji(X),Oa(me,X));const ne=new Set(V.map(ce=>ce.id)),le=v.filter(ce=>!ne.has(ce.id));if(le.length>0){const ve=(await Promise.allSettled(le.map(Z=>yr(p,X,{...Z,sessionId:X,url:P})))).map((Z,gt)=>Z.status==="fulfilled"?Z.value:(console.warn("[Agentation] Failed to sync annotation on reconnect:",Z.reason),le[gt])),_e=[...V,...ve].filter(Yo);y(_e),ys(me,_e,X)}}catch(v){console.warn("[Agentation] Failed to sync on reconnect:",v)}})()},[Mn,p,ue,Ct,me]);const Wp=f.useCallback(()=>{D||(Q(!0),se(!1),M(!1),fe(()=>{Z1(!0),Y(!0),Q(!1)},400))},[D]);f.useEffect(()=>{if(!n||!ue||!e||e.length===0||h.length>0)return;const u=[];return u.push(fe(()=>{M(!0)},t-200)),e.forEach((m,x)=>{const v=t+x*300;u.push(fe(()=>{const S=document.querySelector(m.selector);if(!S)return;const P=S.getBoundingClientRect(),{name:X,path:V}=Or(S),ne={id:`demo-${Date.now()}-${x}`,x:(P.left+P.width/2)/window.innerWidth*100,y:P.top+P.height/2+window.scrollY,comment:m.comment,element:X,elementPath:V,timestamp:Date.now(),selectedText:m.selectedText,boundingBox:{x:P.left,y:P.top+window.scrollY,width:P.width,height:P.height},nearbyText:ms(S),cssClasses:gs(S)};y(le=>[...le,ne])},v))}),()=>{u.forEach(clearTimeout)}},[n,ue,e,t]),f.useEffect(()=>{const u=()=>{Ie(window.scrollY),at(!0),ss.current&&clearTimeout(ss.current),ss.current=fe(()=>{at(!1)},150)};return window.addEventListener("scroll",u,{passive:!0}),()=>{window.removeEventListener("scroll",u),ss.current&&clearTimeout(ss.current)}},[]),f.useEffect(()=>{ue&&h.length>0?Ct?ys(me,h,Ct):bp(me,h):ue&&h.length===0&&localStorage.removeItem($i(me))},[h,me,ue,Ct]),f.useEffect(()=>{if(ue&&!Qe.current){Qe.current=!0;const u=H1(me);u.length>0&&Ge(u)}},[ue,me]),f.useEffect(()=>{ue&&Qe.current&&!$e&&(re.length>0?U1(me,re):Y1(me))},[re,me,ue,$e]),f.useEffect(()=>{if(ue&&!Qi.current){Qi.current=!0;const u=X1(me);if(u){const m={...u,sections:u.sections.map(x=>({...x,currentRect:x.currentRect??{...x.originalRect}}))};pn(m)}}},[ue,me]),f.useEffect(()=>{ue&&Qi.current&&!$e&&(xe?Q1(me,xe):V1(me))},[xe,me,ue,$e]);const oa=f.useRef(!1);f.useEffect(()=>{if(ue&&!oa.current){oa.current=!0;const u=K1(me);u&&(Do.current={rearrange:u.rearrange,placements:u.placements||[]},u.purpose&&Gn(u.purpose))}},[ue,me]),f.useEffect(()=>{if(!ue||!oa.current)return;const u=Do.current;$e?(xe?.sections?.length??0)>0||re.length>0||mt?z_(me,{rearrange:xe,placements:re,purpose:mt}):Fl(me):(u.rearrange?.sections?.length??0)>0||u.placements.length>0||mt?z_(me,{rearrange:u.rearrange,placements:u.placements,purpose:mt}):Fl(me)},[xe,re,mt,$e,me,ue]),f.useEffect(()=>{ae&&!xe&&pn({sections:[],originalOrder:[],detectedAt:Date.now()})},[ae,xe]),f.useEffect(()=>{if(!p||!Ct)return;const u=Jr.current,m=new Set(re.map(x=>x.id));for(const x of re){if(u.has(x.id))continue;u.set(x.id,"");const v=typeof window<"u"?window.location.pathname+window.location.search+window.location.hash:me;yr(p,Ct,{id:x.id,x:x.x/window.innerWidth*100,y:x.y,comment:`Place ${x.type} at (${Math.round(x.x)}, ${Math.round(x.y)}), ${x.width}×${x.height}px${x.text?` — "${x.text}"`:""}`,element:`[design:${x.type}]`,elementPath:"[placement]",timestamp:x.timestamp,url:v,intent:"change",severity:"important",kind:"placement",placement:{componentType:x.type,width:x.width,height:x.height,scrollY:x.scrollY,text:x.text}}).then(S=>{u.has(x.id)&&u.set(x.id,S.id)}).catch(S=>{console.warn("[Agentation] Failed to sync placement annotation:",S),u.delete(x.id)})}for(const[x,v]of u)m.has(x)||(u.delete(x),v&&mo(p,v).catch(()=>{}))},[re,p,Ct,me]),f.useEffect(()=>{if(!(!p||!Ct))return es.current&&clearTimeout(es.current),es.current=fe(()=>{const u=Zr.current;if(!xe||xe.sections.length===0){for(const[,v]of u)v&&mo(p,v).catch(()=>{});u.clear();return}const m=new Set(xe.sections.map(v=>v.id)),x=typeof window<"u"?window.location.pathname+window.location.search+window.location.hash:me;for(const v of xe.sections){const S=v.originalRect,P=v.currentRect;if(!(Math.abs(S.x-P.x)>1||Math.abs(S.y-P.y)>1||Math.abs(S.width-P.width)>1||Math.abs(S.height-P.height)>1)){const ne=u.get(v.id);ne&&(u.delete(v.id),mo(p,ne).catch(()=>{}));continue}const V=u.get(v.id);V?A_(p,V,{comment:`Move ${v.label} section (${v.tagName}) — from (${Math.round(S.x)},${Math.round(S.y)}) ${Math.round(S.width)}×${Math.round(S.height)} to (${Math.round(P.x)},${Math.round(P.y)}) ${Math.round(P.width)}×${Math.round(P.height)}`}).catch(ne=>{console.warn("[Agentation] Failed to update rearrange annotation:",ne)}):(u.set(v.id,""),yr(p,Ct,{id:v.id,x:P.x/window.innerWidth*100,y:P.y,comment:`Move ${v.label} section (${v.tagName}) — from (${Math.round(S.x)},${Math.round(S.y)}) ${Math.round(S.width)}×${Math.round(S.height)} to (${Math.round(P.x)},${Math.round(P.y)}) ${Math.round(P.width)}×${Math.round(P.height)}`,element:v.selector,elementPath:"[rearrange]",timestamp:Date.now(),url:x,intent:"change",severity:"important",kind:"rearrange",rearrange:{selector:v.selector,label:v.label,tagName:v.tagName,originalRect:S,currentRect:P}}).then(ne=>{u.has(v.id)&&u.set(v.id,ne.id)}).catch(ne=>{console.warn("[Agentation] Failed to sync rearrange annotation:",ne),u.delete(v.id)}))}for(const[v,S]of u)m.has(v)||(u.delete(v),S&&mo(p,S).catch(()=>{}))},300),()=>{es.current&&clearTimeout(es.current)}},[xe,p,Ct,me]);const hr=f.useRef(new Map);f.useLayoutEffect(()=>{const u=xe?.sections??[],m=new Set;if((ae||Oe)&&j)for(const x of u){m.add(x.id);try{const v=document.querySelector(x.selector);if(!v)continue;if(!hr.current.has(x.id)){const S={transform:v.style.transform,transformOrigin:v.style.transformOrigin,opacity:v.style.opacity,position:v.style.position,zIndex:v.style.zIndex,display:v.style.display},P=[];let X=v.parentElement;for(;X&&X!==document.body;){const ne=getComputedStyle(X);(ne.overflow!=="visible"||ne.overflowX!=="visible"||ne.overflowY!=="visible")&&(P.push({el:X,overflow:X.style.overflow}),X.style.overflow="visible"),X=X.parentElement}getComputedStyle(v).display==="inline"&&(v.style.display="inline-block"),hr.current.set(x.id,{el:v,origStyles:S,ancestors:P}),v.style.transformOrigin="top left",v.style.zIndex="9999"}}catch{}}for(const[x,v]of hr.current)if(!m.has(x)){const{el:S,origStyles:P,ancestors:X}=v;S.style.transition="transform 0.4s cubic-bezier(0.22, 1, 0.36, 1), opacity 0.4s cubic-bezier(0.22, 1, 0.36, 1)",S.style.transform=P.transform,S.style.transformOrigin=P.transformOrigin,S.style.opacity=P.opacity,S.style.position=P.position,S.style.zIndex=P.zIndex,hr.current.delete(x),fe(()=>{S.style.transition="",S.style.display=P.display;for(const V of X)V.el.style.overflow=V.overflow},450)}},[xe,ae,Oe,j]),f.useEffect(()=>()=>{for(const[,u]of hr.current){const{el:m,origStyles:x,ancestors:v}=u;m.style.transition="transform 0.4s cubic-bezier(0.22, 1, 0.36, 1), opacity 0.4s cubic-bezier(0.22, 1, 0.36, 1)",m.style.transform=x.transform,m.style.transformOrigin=x.transformOrigin,m.style.opacity=x.opacity,m.style.position=x.position,m.style.zIndex=x.zIndex,fe(()=>{m.style.transition="",m.style.display=x.display;for(const S of v)S.el.style.overflow=S.overflow},450)}hr.current.clear()},[]);const fl=f.useCallback(()=>{Le(!0),We(!1),pe(null),clearTimeout(al.current),al.current=fe(()=>{Le(!1)},300)},[]),ud=f.useCallback(()=>{ae&&(Le(!0),We(!1),pe(null),clearTimeout(al.current),al.current=fe(()=>{Le(!1)},300)),M(!1)},[ae]),dd=f.useCallback(()=>{E||(Qg(),B(!0))},[E]),hl=f.useCallback(()=>{E&&(S_(),B(!1))},[E]),ra=f.useCallback(()=>{E?hl():dd()},[E,dd,hl]),_d=f.useCallback(()=>{if(bt.length===0)return;const u=bt[0],m=u.element,x=bt.length>1,v=bt.map(S=>S.element.getBoundingClientRect());if(x){const S={left:Math.min(...v.map(Z=>Z.left)),top:Math.min(...v.map(Z=>Z.top)),right:Math.max(...v.map(Z=>Z.right)),bottom:Math.max(...v.map(Z=>Z.bottom))},P=bt.slice(0,5).map(Z=>Z.name).join(", "),X=bt.length>5?` +${bt.length-5} more`:"",V=v.map(Z=>({x:Z.left,y:Z.top+window.scrollY,width:Z.width,height:Z.height})),le=bt[bt.length-1].element,ce=v[v.length-1],ve=ce.left+ce.width/2,Te=ce.top+ce.height/2,_e=Xa(le);U({x:ve/window.innerWidth*100,y:_e?Te:Te+window.scrollY,clientY:Te,element:`${bt.length} elements: ${P}${X}`,elementPath:"multi-select",boundingBox:{x:S.left,y:S.top+window.scrollY,width:S.right-S.left,height:S.bottom-S.top},isMultiSelect:!0,isFixed:_e,elementBoundingBoxes:V,multiSelectElements:bt.map(Z=>Z.element),targetElement:le,fullPath:Ol(m),accessibility:zl(m),computedStyles:Bl(m),computedStylesObj:Dl(m),nearbyElements:Pl(m),cssClasses:gs(m),nearbyText:ms(m),sourceFile:Yl(m)})}else{const S=v[0],P=Xa(m);U({x:S.left/window.innerWidth*100,y:P?S.top:S.top+window.scrollY,clientY:S.top,element:u.name,elementPath:u.path,boundingBox:{x:S.left,y:P?S.top:S.top+window.scrollY,width:S.width,height:S.height},isFixed:P,fullPath:Ol(m),accessibility:zl(m),computedStyles:Bl(m),computedStylesObj:Dl(m),nearbyElements:Pl(m),cssClasses:gs(m),nearbyText:ms(m),reactComponents:u.reactComponents,sourceFile:Yl(m)})}ur([]),Ne(null)},[bt]);f.useEffect(()=>{j||(U(null),ie(null),ge(null),Ye([]),Ne(null),se(!1),ur([]),zn.current={cmd:!1,shift:!1},E&&hl())},[j,E,hl]),f.useEffect(()=>()=>{S_()},[]),f.useEffect(()=>{if(!j)return;const u=["p","span","h1","h2","h3","h4","h5","h6","li","td","th","label","blockquote","figcaption","caption","legend","dt","dd","pre","code","em","strong","b","i","u","s","a","time","address","cite","q","abbr","dfn","mark","small","sub","sup","[contenteditable]"].join(", "),m=":not([data-agentation-root]):not([data-agentation-root] *)",x=document.createElement("style");return x.id="feedback-cursor-styles",x.textContent=`
      body ${m} {
        cursor: crosshair !important;
      }
