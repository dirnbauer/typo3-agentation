...
.icon-transitions-module__sending___uaLN- {
  ...
  `,document.head.appendChild(e),Re.pausedAnimations=[];try{document.getAnimations().forEach(t=>{if(t.playState!=="running")return;const n=t.effect?.target;Xg(n)||(t.pause(),Re.pausedAnimations.push(t))})}catch{}document.querySelectorAll("video").forEach(t=>{t.paused||(t.dataset.wasPaused="false",t.pause())})}function S_(){if(typeof document>"u"||!Re.frozen)return;Re.frozen=!1;const e=Re.frozenTimeoutQueue;Re.frozenTimeoutQueue=[];for(const n of e)Re.origSetTimeout(()=>{if(Re.frozen){Re.frozenTimeoutQueue.push(n);return}try{n()}catch(o){console.warn("[agentation] Error replaying queued timeout:",o)}},0);const t=Re.frozenRAFQueue;Re.frozenRAFQueue=[];for(const n of t)Re.origRAF(o=>{if(Re.frozen){Re.frozenRAFQueue.push(n);return}n(o)});for(const n of Re.pausedAnimations)try{n.play()}catch(o){console.warn("[agentation] Error resuming animation:",o)}Re.pausedAnimations=[],document.getElementById(Oc)?.remove(),document.querySelectorAll("video").forEach(n=>{n.dataset.wasPaused==="false"&&(n.play().catch(()=>{}),delete n.dataset.wasPaused)})}function Ba(e){if(!e)return;const t=n=>n.stopImmediatePropagation();document.addEventListener("focusin",t,!0),document.addEventListener("focusout",t,!0);try{e.focus()}finally{document.removeEventListener("focusin",t,!0),document.removeEventListener("focusout",t,!0)}}var Ei=f.forwardRef(function({element:t,timestamp:n,selectedText:o,placeholder:r="What should change?",initialValue:l="",submitLabel:i="Add",onSubmit:a,onCancel:c,onDelete:d,style:p,accentColor:x="#3c82f7",isExiting:g=!1,lightMode:$=!1,computedStyles:k},j){const[M,h]=f.useState(l),[y,b]=f.useState(!1),[L,W]=f.useState("initial"),[Y,D]=f.useState(!1),[Q,he]=f.useState(!1),q=f.useRef(null),de=f.useRef(null),Be=f.useRef(null),Ae=f.useRef(null);f.useEffect(()=>{g&&L!=="exit"&&W("exit")},[g,L]),f.useEffect(()=>{fe(()=>{W("enter")},0);const U=fe(()=>{W("entered")},200),G=fe(()=>{const Se=q.current;Se&&(Ba(Se),Se.selectionStart=Se.selectionEnd=Se.value.length,Se.scrollTop=Se.scrollHeight)},50);return()=>{clearTimeout(U),clearTimeout(G),Be.current&&clearTimeout(Be.current),Ae.current&&clearTimeout(Ae.current)}},[]);const ke=f.useCallback(()=>{Ae.current&&clearTimeout(Ae.current),b(!0),Ae.current=fe(()=>{b(!1),Ba(q.current)},250)},[]);f.useImperativeHandle(j,()=>({shake:ke}),[ke]);const Le=f.useCallback(()=>{W("exit"),Be.current=fe(()=>{c()},150)},[c]),oe=f.useCallback(()=>{M.trim()&&a(M.trim())},[M,a]),lt=f.useCallback(U=>{U.stopPropagation(),!U.nativeEvent.isComposing&&(U.key==="Enter"&&!U.shiftKey&&(U.preventDefault(),oe()),U.key==="Escape"&&Le())},[oe,Le]),I=[nt.popup,$?nt.light:"",L==="enter"?nt.enter:"",L==="entered"?nt.entered:"",L==="exit"?nt.exit:"",y?nt.shake:""].filter(Boolean).join(" ");return s.jsxs("div",{ref:de,className:I,"data-annotation-popup":!0,style:p,onClick:U=>U.stopPropagation(),children:[s.jsxs("div",{className:nt.header,children:[k&&Object.keys(k).length>0?s.jsxs("button",{className:nt.headerToggle,onClick:()=>{const U=Q;he(!Q),U&&fe(()=>Ba(q.current),0)},type:"button",children:[s.jsx("svg",{className:`${nt.chevron} ${Q?nt.expanded:""}`,width:"14",height:"14",viewBox:"0 0 14 14",fill:"none",xmlns:"http://www.w3.org/2000/svg",children:s.jsx("path",{d:"M5.5 10.25L9 7.25L5.75 4",stroke:"currentColor",strokeWidth:"1.5",strokeLinecap:"round",strokeLinejoin:"round"})}),s.jsx("span",{className:nt.element,children:t})]}):s.jsx("span",{className:nt.element,children:t}),n&&s.jsx("span",{className:nt.timestamp,children:n})]}),k&&Object.keys(k).length>0&&s.jsx("div",{className:`${nt.stylesWrapper} ${Q?nt.expanded:""}`,children:s.jsx("div",{className:nt.stylesInner,children:s.jsx("div",{className:nt.stylesBlock,children:Object.entries(k).map(([U,G])=>s.jsxs("div",{className:nt.styleLine,children:[s.jsx("span",{className:nt.styleProperty,children:U.replace(/([A-Z])/g,"-$1").toLowerCase()}),": ",s.jsx("span",{className:nt.styleValue,children:G}),";"]},U))})})}),o&&s.jsxs("div",{className:nt.quote,children:["“",o.slice(0,80),o.length>80?"...":"","”"]}),s.jsx("textarea",{ref:q,className:nt.textarea,style:{borderColor:Y?x:void 0},placeholder:r,value:M,onChange:U=>h(U.target.value),onFocus:()=>D(!0),onBlur:()=>D(!1),rows:2,onKeyDown:lt}),s.jsxs("div",{className:nt.actions,children:[d&&s.jsx("div",{className:nt.deleteWrapper,children:s.jsx("button",{className:nt.deleteButton,onClick:d,type:"button",children:s.jsx(Wg,{size:22})})}),s.jsx("button",{className:nt.cancel,onClick:Le,children:"Cancel"}),s.jsx("button",{className:nt.submit,style:{backgroundColor:x,opacity:M.trim()?1:.4},onClick:oe,disabled:!M.trim(),children:i})]})]})}),Vg=({content:e,children:t,...n})=>{const[o,r]=f.useState(!1),[l,i]=f.useState(!1),[a,c]=f.useState({top:0,right:0}),d=f.useRef(null),p=f.useRef(null),x=f.useRef(null),g=()=>{if(d.current){const j=d.current.getBoundingClientRect();c({top:j.top+j.height/2,right:window.innerWidth-j.left+8})}},$=()=>{i(!0),x.current&&(clearTimeout(x.current),x.current=null),g(),p.current=fe(()=>{r(!0)},500)},k=()=>{p.current&&(clearTimeout(p.current),p.current=null),r(!1),x.current=fe(()=>{i(!1)},150)};return f.useEffect(()=>()=>{p.current&&clearTimeout(p.current),x.current&&clearTimeout(x.current)},[]),s.jsxs(s.Fragment,{children:[s.jsx("span",{ref:d,onMouseEnter:$,onMouseLeave:k,...n,children:t}),l&&Ou.createPortal(s.jsx("div",{"data-feedback-toolbar":!0,style:{position:"fixed",top:a.top,right:a.right,transform:"translateY(-50%)",padding:"6px 10px",background:"#383838",color:"rgba(255, 255, 255, 0.7)",fontSize:"11px",fontWeight:400,lineHeight:"14px",borderRadius:"10px",width:"180px",textAlign:"left",zIndex:100020,pointerEvents:"none",boxShadow:"0px 1px 8px rgba(0, 0, 0, 0.28)",opacity:o?1:0,transition:"opacity 0.15s ease"},children:e}),document.body)]})},Kg=`.styles-module__tooltip___mcXL2 {
  display: flex;
  justify-content: center;
  align-items: center;
  cursor: help;
...
`,r.forEach((M,h)=>{const y=Ln[M.type]?.label||M.type;l+=`${h+1}. **${y}** — \`${Math.round(M.width)}×${Math.round(M.height)}px\` at \`(${Math.round(M.x)}, ${Math.round(M.y)})\`
`}),l;const i=yp(t);l+=vp(i),l+=`### Components
`,r.forEach((M,h)=>{const y=Ln[M.type]?.label||M.type,b={x:M.x,y:M.y,width:M.width,height:M.height};l+=`${h+1}. **${y}** — \`${Math.round(M.width)}×${Math.round(M.height)}px\` at \`(${Math.round(M.x)}, ${Math.round(M.y)})\`
`;const L=Fc(b),Y=gp(L,{includeLeftRight:o==="detailed"||o==="forensic"});for(const Q of Y)l+=`   - ${Q}
`;const D=xp(b,i);D&&(l+=`   - CSS: ${D}
`)}),l+=`
### Layout Analysis
`;const a=[];for(const M of r){const h=a.find(y=>Math.abs(y.y-M.y)<30);h?h.items.push(M):a.push({y:M.y,items:[M]})}if(a.sort((M,h)=>M.y-h.y),a.forEach((M,h)=>{M.items.sort((b,L)=>b.x-L.x);const y=M.items.map(b=>Ln[b.type]?.label||b.type);if(M.items.length===1){const L=M.items[0].width>t.width*.8;l+=`- Row ${h+1} (y≈${Math.round(M.y)}): ${y[0]}${L?" — full width":""}
`}else l+=`- Row ${h+1} (y≈${Math.round(M.y)}): ${y.join(" | ")} — ${M.items.length} items side by side
`}),o==="detailed"||o==="forensic"){l+=`
### Spacing & Gaps
`;for(let M=0;M<r.length-1;M++){const h=r[M],y=r[M+1],b=Ln[h.type]?.label||h.type,L=Ln[y.type]?.label||y.type,W=Math.round(y.y-(h.y+h.height)),Y=Math.round(y.x-(h.x+h.width));Math.abs(h.y-y.y)<30?l+=`- ${b} → ${L}: \`${Y}px\` horizontal gap
`:l+=`- ${b} → ${L}: \`${W}px\` vertical gap
`}if(o==="forensic"&&r.length>2){l+=`
### All Pairwise Gaps
`;for(let M=0;M<r.length;M++)for(let h=M+1;h<r.length;h++){const y=r[M],b=r[h],L=Ln[y.type]?.label||y.type,W=Ln[b.type]?.label||b.type,Y=Math.round(b.y-(y.y+y.height)),D=Math.round(b.x-(y.x+y.width));l+=`- ${L} ↔ ${W}: h=\`${D}px\` v=\`${Y}px\`
`}}o==="forensic"&&(l+=`
### Z-Order (placement order)
`,e.forEach((M,h)=>{const y=Ln[M.type]?.label||M.type;l+=`${h}. ${y} at \`(${Math.round(M.x)}, ${Math.round(M.y)})\`
`}))}l+=`
...
`;const b=g?{width:k.width,height:k.height}:void 0,L=`at (${Math.round(k.x)}, ${Math.round(k.y)})`,W=b?`, ${Math.round(b.width)}×${Math.round(b.height)}px`:"",D=gp(h,{includeLeftRight:t==="detailed"||t==="forensic"});if(D.length>0){l+=`  - Suggested position ${L}${W}: ${D[0]}
`;for(let he=1;he<D.length;he++)l+=`    ${D[he]}
`}else l+=`  - Suggested position ${L}${W}
...
.styles-module__checkboxContainer___joqZk:has(.styles-module__checkboxInput___ECzzO:checked) .styles-module__checkboxCheckPath___cDyh8 {
  ...
  stroke-dashoffset: 0;
}`,Nx={checkboxContainer:"styles-module__checkboxContainer___joqZk",checkboxInput:"styles-module__checkboxInput___ECzzO",checkboxCheck:"styles-module__checkboxCheck___fUXpr",checkboxCheckPath:"styles-module__checkboxCheckPath___cDyh8"};if(typeof document<"u"){let e=document.getElementById("feedback-tool-styles-checkbox-styles");e||(e=document.createElement("style"),e.id="feedback-tool-styles-checkbox-styles",document.head.appendChild(e)),e.textContent=Lx}var Ul=Nx,Rx=({className:e="",...t})=>s.jsxs("div",{className:`${Ul.checkboxContainer} ${e}`,children:[s.jsx("input",{className:Ul.checkboxInput,type:"checkbox",...t}),s.jsx("svg",{className:Ul.checkboxCheck,width:"14",height:"14",viewBox:"0 0 14 14",fill:"none",children:s.jsx("path",{className:Ul.checkboxCheckPath,d:"M3.94 7L6.13 9.19L10.5 4.81",stroke:"currentColor",strokeWidth:"1.5",strokeLinecap:"round",strokeLinejoin:"round"})})]}),Tx=`.styles-module__container___w8eAF {
  display: flex;
  align-items: center;
  height: 24px;
}
